...
public class Character : NetworkBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (posRightCrosshairCenter.x > Screen.width / 2 - Screen.width * (centerCam.GetComponent<Camera>().rect.x / 2))
        {
    ...
    void CalcShoot(SHOOT_TYPE st)
    {
        ...
        else if (st == SHOOT_TYPE.RIGHT_GATLING)
        {
            ...
            Vector3 shootDir = p - shootSpawnRight.position;
            shootDir.Normalize();
            CmdShootGatling(shootSpawnRight.position, RightArmTransform.rotation, shootDir,1);
        }
        ...
        else if (st == SHOOT_TYPE.LEFT_GATLING)
        {
            ...
            Vector3 shootDir = p - shootSpawnLeft.position;
            shootDir.Normalize();
            Debug.Log(LeftArmTransform.rotation.eulerAngles);
            CmdShootGatling(shootSpawnLeft.position, LeftArmTransform.rotation, shootDir, 0);
        }
    ...
    [Command]
    void CmdShootGatling(Vector3 position, Quaternion rotation, Vector3 direction, int armID)
    {
        RaycastHit hit;
        if (Physics.Raycast(position, direction, out hit))
        {
            if (hit.collider != null)
            {
                ...
                if ( armID == 0)
                {
                    armTemp = transform.FindChild("LeftArm");
                }
                else
                {
                    armTemp = transform.FindChild("RightArm");
                }
                ImpactClone = (GameObject)Instantiate(GatlingParticleEffect, position, armTemp.rotation);
                NetworkServer.Spawn(ImpactClone);
                Destroy(ImpactClone, 0.2f);
